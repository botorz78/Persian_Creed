...
local function run(msg, matches)
	if msg.to.type == 'chat' then
		if matches[1]:lower() == 'تغیر به سوپر گروه' then
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1]:lower() == 'افزودن' and not matches[2] then
		...
		if matches[1]:lower() == 'حذف' and is_admin1(msg) and not matches[2] then
		...
		if matches[1]:lower() == "لیست ادمین ها" then
		...
		if matches[1]:lower() == "صاحب گروه" then
		...
		if matches[1]:lower() == "لیست مدیران" then
		...
		if matches[1]:lower() == "امار" and not matches[2] and is_momod(msg) then
			local user_id = msg.from.peer_id
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested SuperGroup users list")
			channel_get_users(receiver, callback_who, {receiver = receiver})
		end

		if matches[1]:lower() == "اخراج شدگان" and is_momod(msg) then
		...
		if matches[1]:lower() == 'بلاک' and is_momod(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
		...
		if matches[1]:lower() == 'ایدی' then
			if type(msg.reply_id) ~= "nil" and is_momod(msg) and not matches[2] then
				...
				get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif type(msg.reply_id) ~= "nil" and matches[2] == "محل" and is_momod(msg) then
				local cbreply_extra = {
					get_cmd = 'idfrom',
					msg = msg
				}
		...
		if matches[1]:lower() == 'اخراجم کن' then
			if msg.to.type == 'channel' then
		...
		if matches[1]:lower() == 'ذخیره لینک' and is_owner(msg) then
		...
		if matches[1]:lower() == "دعوت" and is_sudo(msg) then
			local cbres_extra = {
		...
		if matches[1]:lower() == 'اطلاعات' and is_owner(msg) then
			local cbres_extra = {
				channelid = msg.to.id,
				get_cmd = 'res'
			}
		...
		--[[if matches[1]:lower() == 'اخراج' and is_momod(msg) then
			local receiver = channel..matches[3]
			local user = "user#id"..matches[2]
			chaannel_kick(receiver, user, ok_cb, false)
		end]]

			if matches[1]:lower() == 'ترفیع به ادمین' then
		...
		if matches[1]:lower() == 'ازل مقام ادمین' then
			if not is_support(msg.from.id) and not is_owner(msg) then
				return
			end
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1]:lower() == 'ازل مقام ادمین' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1]:lower() == 'ترفیع به صاحب گروه' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
			...
			elseif matches[1]:lower() == 'ترفیع به صاحب گروه' and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
		...
		if matches[1]:lower() == 'ترفیع به کمک مدیر' then
		  if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
			...
			elseif matches[1]:lower() == 'ترفیع به کمک مدیر' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1]:lower() == 'ازل مقام کمک مدیر' then
			if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
			...
			elseif matches[1]:lower() == 'ازل مقام کمک مدیر' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1]:lower() == "ذخیره اسم" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1]:lower() == "ذخیره موضوع" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1]:lower() == 'ذخیره قوانین' and is_momod(msg) then
			rules = matches[2]
			local target = msg.to.id
		...
		if matches[1]:lower() == 'عکس' and is_momod(msg) then
		...
		if matches[1]:lower() == 'قفل کردن' and is_momod(msg) then
			local target = msg.to.id
		...
		if matches[1]:lower() == 'باز کردن' and is_momod(msg) then
			local target = msg.to.id
			if matches[2] == 'تبلیغات' then
		...
		if matches[1]:lower() == 'حساسیت' then
			if not is_momod(msg) then
				return
			end
		...
		if matches[1]:lower() == 'عمومی' and is_momod(msg) then
			local target = msg.to.id
		...
		if matches[1]:lower() == 'سایلنت' and is_owner(msg) then
			local chat_id = msg.to.id
		...
		if matches[1]:lower() == "سایلنت کاربر" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1]:lower() == "سایلنت کاربر" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1]:lower() == "لیست ابزار سایلنت" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1]:lower() == "لیست افراد سایلنت" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1]:lower() == 'راهنما' and not is_owner(msg) then
			text = "Message /superhelp to @viper_tg in private for SuperGroup help"
